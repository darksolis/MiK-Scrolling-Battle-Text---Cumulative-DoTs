...
frame:Hide()
MSBTOptMain.AddTab(frame,"Cumulative DoTs","Configure premium persistent cumulative periodic-damage text.")

local built=false
local buildErrorText
local function BuildCumulativeDotTab()
 local API=MikSBT and MikSBT.CumulativeDots
 if not API then
  local fs=frame:CreateFontString(nil,"OVERLAY","GameFontNormal")
  fs:SetPoint("TOPLEFT",frame,"TOPLEFT",8,-8)
  fs:SetWidth(350); fs:SetJustifyH("LEFT")
  fs:SetText("The Cumulative DoT tracker module is not loaded. Check that MSBTCumulativeDots.lua is listed in MikScrollingBattleText.toc.")
  return
 end
local controls={}
...
local function Label(parent,text,x,y,template,width)
...
local function Divider(y,title)
 local line=content:CreateTexture(nil,"ARTWORK"); line:SetTexture(1,1,1,.12); line:SetPoint("TOPLEFT",0,y-20); line:SetPoint("TOPRIGHT",-4,y-20); line:SetHeight(1)
 Label(content,title,0,y,"GameFontNormalLarge")
end
local function Check(text,x,y,key,tip)
 local b=CreateFrame("CheckButton",nil,content,"UICheckButtonTemplate"); b:SetPoint("TOPLEFT",x,y); b:SetWidth(24); b:SetHeight(24); b.key=key
 local fs=content:CreateFontString(nil,"OVERLAY","GameFontNormalSmall"); fs:SetPoint("LEFT",b,"RIGHT",0,1); fs:SetText(text)
 b:SetScript("OnClick",function(self) local d=DB(); if d then d[key]=self:GetChecked() and true or false; RefreshTracker(); frame:Refresh() end end)
 if tip then b:SetScript("OnEnter",function(self) GameTooltip:SetOwner(self,"ANCHOR_RIGHT"); GameTooltip:SetText(text); GameTooltip:AddLine(tip,1,1,1,true); GameTooltip:Show() end); b:SetScript("OnLeave",function() GameTooltip:Hide() end) end
 controls[#controls+1]=b; return b
end
local function Button(text,x,y,w,fn)
 local b=CreateFrame("Button",nil,content,"UIPanelButtonTemplate"); b:SetPoint("TOPLEFT",x,y); b:SetWidth(w); b:SetHeight(22); b:SetText(text); b:SetScript("OnClick",fn); return b
end
local function Drop(label,x,y,w,key,items,tip)
 local d=MSBTControls.CreateDropdown(content); d:Configure(w,label,tip or ""); d:SetPoint("TOPLEFT",content,"TOPLEFT",x,y)
 for _,it in ipairs(items) do d:AddItem(it[2],it[1]) end
 d:SetChangeHandler(function(self,id) local db=DB(); if db then db[key]=id; RefreshTracker(); frame:Refresh() end end)
 d.dbKey=key; controls[#controls+1]=d; return d
end
local function Stepper(label,x,y,key,minv,maxv,step,suffix)
 Label(content,label,x,y)
 local value=Label(content,"",x+88,y,"GameFontHighlightSmall",64); value:SetJustifyH("CENTER")
 Button("-",x,y-22,30,function() local d=DB(); d[key]=math.max(minv,(tonumber(d[key]) or minv)-step); RefreshTracker(); frame:Refresh() end)
 Button("+",x+122,y-22,30,function() local d=DB(); d[key]=math.min(maxv,(tonumber(d[key]) or minv)+step); RefreshTracker(); frame:Refresh() end)
 local holder={key=key,value=value,suffix=suffix or ""}; controls[#controls+1]=holder; return holder
end

Label(content,"Cumulative DoT Tracker",0,0,"GameFontNormalLarge")
Label(content,"Persistent periodic-damage totals rendered as native MSBT text.",0,-24,"GameFontHighlightSmall",350)

-- Live preview
local previewBox=CreateFrame("Frame",nil,content); previewBox:SetPoint("TOPLEFT",0,-54); previewBox:SetWidth(350); previewBox:SetHeight(82)
previewBox:SetBackdrop({bgFile="Interface\\Tooltips\\UI-Tooltip-Background",edgeFile="Interface\\Tooltips\\UI-Tooltip-Border",tile=true,tileSize=16,edgeSize=12,insets={left=3,right=3,top=3,bottom=3}})
previewBox:SetBackdropColor(0,0,0,.55); previewBox:SetBackdropBorderColor(.45,.45,.45,.8)
Label(previewBox,"LIVE PREVIEW",10,-8,"GameFontNormalSmall")
local preview=previewBox:CreateFontString(nil,"OVERLAY","GameFontNormal")
preview:SetPoint("CENTER",previewBox,"CENTER",0,-8); preview:SetWidth(330); preview:SetJustifyH("CENTER")
-- Wrath/3.3.5 throws "Font not set" if SetText is called on a bare FontString.
-- Give the preview a guaranteed fallback font before assigning any text.
preview:SetFont("Fonts\\FRIZQT__.TTF",26,"THICKOUTLINE")
preview:SetText("Corruption 73.5k • Immolate 55.0k   128.5k")

Divider(-150,"Tracking")
Check("Enable tracker",0,-180,"enabled","Master switch for cumulative DoT text.")
Check("Track all periodic damage",0,-208,"allPeriodic","Turn this off to use the whitelist.")
Check("Include pet damage",180,-180,"includePet","Includes periodic damage from your active pet.")
Check("Clear when combat ends",180,-208,"clearOnCombatEnd","Useful when the server misses death events.")
local mode=Drop("Reset total",0,-246,165,"mode",{{"death","Target death"},{"application","DoT expires"}})
local display=Drop("Multiple targets",180,-246,165,"displayMode",{{"targets","Separate totals"},{"combined","Combined total"}})
local sort=Drop("Target sorting",0,-300,165,"sortMode",{{"recent","Most recent"},{"damage","Highest damage"},{"name","Target name"}})
local area=Drop("MSBT scroll area",180,-300,165,"scrollArea",{{"Outgoing","Outgoing"},{"Incoming","Incoming"},{"Notification","Notification"},{"Static","Static"}})

Divider(-370,"Text Style")
Check("Use scroll area's font",0,-400,"inheritFont")
local fontDrop=MSBTControls.CreateDropdown(content); fontDrop:Configure(345,"Font","Choose from all fonts registered with MSBT."); fontDrop:SetPoint("TOPLEFT",content,"TOPLEFT",0,-438)
fontDrop:SetListboxHeight(220); fontDrop:SetListboxWidth(345)
fontDrop:SetChangeHandler(function(self,id) local d=DB(); if d then d.fontName=id; d.inheritFont=false; RefreshTracker(); frame:Refresh() end end)
fontDrop.dbKey="fontName"; controls[#controls+1]=fontDrop
local fonts=API.GetFonts and API.GetFonts() or {}; for _,name in ipairs(fonts) do fontDrop:AddItem(name,name) end
local size=Stepper("Text size",0,-492,"fontSize",12,64,1," px")
local spacing=Stepper("Row spacing",180,-492,"spacing",0,30,1," px")
local outline=Drop("Outline",0,-548,165,"outline",{{"","None"},{"OUTLINE","Outline"},{"THICKOUTLINE","Thick"},{"MONOCHROME","Monochrome"}})
local align=Drop("Alignment",180,-548,165,"alignment",{{"LEFT","Left"},{"CENTER","Center"},{"RIGHT","Right"}})
Button("Text color",0,-606,95,function()
 local d=DB(); if not d then return end
 ColorPickerFrame.func=function() local r,g,b=ColorPickerFrame:GetColorRGB(); d.colorR=math.floor(r*255+.5); d.colorG=math.floor(g*255+.5); d.colorB=math.floor(b*255+.5); RefreshTracker(); frame:Refresh() end
 ColorPickerFrame.cancelFunc=function(prev) d.colorR=math.floor(prev.r*255+.5); d.colorG=math.floor(prev.g*255+.5); d.colorB=math.floor(prev.b*255+.5); RefreshTracker(); frame:Refresh() end
 ColorPickerFrame:SetColorRGB((d.colorR or 255)/255,(d.colorG or 190)/255,(d.colorB or 35)/255); ColorPickerFrame:Show()
end)
Check("Abbreviate numbers",110,-606,"compactNumbers")
Check("Pulse on critical ticks",0,-634,"pulseOnCrit")

Divider(-688,"Labels & Detail")
Check("Show target names",0,-718,"showTargetName")
Check("Show spell icons",180,-718,"showSpellIcons")
Check("Only list active DoTs",0,-746,"activeSpellsOnly")
Check("Add 'damage' label",180,-746,"showDamageLabel")
local spellMode=Drop("Spell information",0,-786,165,"spellLabelMode",{{"none","Damage only"},{"names","Spell names"},{"breakdown","Names + totals"}})
local maxSpells=Stepper("Max spells shown",180,-786,"maxSpellLabels",1,10,1,"")
...
function frame:Refresh()
 ...
 for _,c in ipairs(controls) do
  if c.GetObjectType and c:GetObjectType()=="CheckButton" and c.key then c:SetChecked(d[c.key] and true or false)
  elseif c.dbKey and c.SetSelectedID then c:SetSelectedID(d[c.dbKey])
  elseif c.value and c.key then local v=d[c.key] or 0; if type(v)=="number" and v~=math.floor(v) then v=string.format("%.2f",v):gsub("0+$",""):gsub("%.$","") end; c.value:SetText(tostring(v)..c.suffix) end
 end
 ...
 preview:SetJustifyH(d.alignment or "CENTER"); preview:SetText(PreviewText(d))
 local count=0; for _ in pairs(d.whitelist or {}) do count=count+1 end; status:SetText(count.." saved")
 RefreshTracker()
end

 frame:SetScript("OnShow",function(self) if self.Refresh then self:Refresh() end end)
end

frame:SetScript("OnShow",function(self)
 if not built then
  built=true
  local ok,err=pcall(BuildCumulativeDotTab)
  if not ok then
   buildErrorText=self:CreateFontString(nil,"OVERLAY","GameFontNormal")
   buildErrorText:SetPoint("TOPLEFT",self,"TOPLEFT",8,-8)
   buildErrorText:SetWidth(350); buildErrorText:SetJustifyH("LEFT")
   buildErrorText:SetText("Cumulative DoT options failed to build: "..tostring(err))
  end
 end
